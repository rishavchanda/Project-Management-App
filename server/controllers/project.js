...
export const inviteProjectMember = async (req, res, next) => {
  ...
  const link = `http://localhost:3001/projects/invite/${req.app.locals.CODE}?projectid=${req.params.id}&userid=${req.body.id}&access=${req.body.access}&role=${req.body.role}`;
  const mailBody = `
